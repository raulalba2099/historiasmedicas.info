...
class RecetaController extends Controller
{
    ...
    static public function mostrarRecetas($id)
    {

        $recetas = Receta::join('citas', 'cit_id', 'rec_cit_id')
            ->join('pacientes', 'cit_pac_id', 'pac_id')->where('pac_id', $id)->get();

        $recetaCita = Receta::join('citas', 'cit_id', 'rec_cit_id')
            ->join('pacientes', 'cit_pac_id', 'pac_id')->where('pac_id', $id)->get();
        ...
        foreach ($recetas as $receta) {
          $recetasArray[$receta['cit_id']]['cit_id'] = $receta->cit_id;
          $recetasArray[$receta['cit_id']]['receta'][] = $receta;
        }
        $cantidadRegistros = 0;
        if (count($recetas) > 0) {
            $statusReceta = 200;
            $cantidadRegistros = count($recetas);
        } else {
            $statusReceta = 400;
        }
        ...
        foreach ($recetas as $key => $receta) {
            $fechaReceta = new DateTime(date($receta->cit_fecha));

            $intervaloFechas = $fechaActual->diff($fechaReceta);
            $horasDiasRecetas = $intervaloFechas->days * 24;

            $recetasArray[$key]['id'] = $receta->rec_id;
            $recetasArray[$key]['fecha'] = $receta->cit_fecha;
            $recetasArray[$key]['horas'] = $horasDiasRecetas;
        }
...
//        dd($recetasArray);
        ...
        return view(
            "reportes.expediente-recetas",
            array(
                'cantidadRegistros' => $cantidadRegistros,
                'statusReceta' => $statusReceta,
                'recetas' => $recetas,
                'recetasCalendar' => $recetasArray,
                'recetaCita' => $recetaCita,
                'paciente' =>$paciente
            )
        );
    }

    static public function mostrarReceta($id, $fecha)
    {


        $fechaFormato = new DateTime($fecha);
        $fechaConsulta = $fechaFormato->format('Y-m-d');


        $medicamentos = Receta::join('citas', 'cit_id', 'rec_cit_id')
            ->join('pacientes', 'cit_pac_id', 'pac_id')->where('pac_id', $id)
            ->where('cit_fecha', $fechaConsulta)->get();

        $paciente = Paciente::where('pac_id', $id)->get()->toArray();

//        dd($medicamentos);

       return array('status' => 1, 'medicamentos' =>$medicamentos, 'fecha' => $fechaFormato->format('d-M-y'));
...
//        );
    ...
    static public function mostrarPacientes()
    {
...
//        $pdf->Ln();
        ...
        $th = array("#", "Medicamento", "Dosis", utf8_decode('Duración'));
